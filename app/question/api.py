...
async def save_user_answer(user_answer_request: schemas.UserAnswerRequest, db=Depends(get_db)):
    ...
    if existing_user_answer:
        ...
        raise HTTPException(status_code=400, detail="User answer for this pair_id and user_id already exists")

    db_user_answer = models.UserAnswer(user_id=user_id, pair_id=pair_id, answer_json=answer_json_dict, sort_order=sort_order)
...
def generatearticle(answer_json_dict: schemas.Ranking):
        ...
        client = openapi(api_key="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")
